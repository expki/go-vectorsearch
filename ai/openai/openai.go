...
func New(cfg config.AI) (ai *OpenAI, err error) {
	...
	// Parse Embed URI
	if cfg.Embed != nil {
		logger.Sugar().Info("Adding AI provider: OpenAI-Embed")
	...
	// Parse Chat URI
	if cfg.Chat != nil {
		logger.Sugar().Info("Adding AI provider: OpenAI-Chat")
	...
	// Parse Generate URI
	if cfg.Generate != nil {
		logger.Sugar().Info("Adding AI provider: OpenAI-Generate")
...
func (ai *OpenAI) CanEmbed() bool {
	return ai.embed != nil
}

func (ai *OpenAI) CanChat() bool {
	return ai.chat != nil
}

func (ai *OpenAI) CanGenerate() bool {
